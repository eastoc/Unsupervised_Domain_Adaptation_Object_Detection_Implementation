...
class ResNet_DA_Tri_Att(BaseModule):
    """ResNet DA backbone.
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(ResNet_DA_Tri_Att, self).__init__(init_cfg)
    ...
    def forward_train(self, x, gt_domain):
        ...
        for i, layer_name in enumerate(self.res_layers):
            ...
            if i in self.da_outs_idx:
                ...
                if i == 1:
                    if self.context:
                        local_feat_bottom, feat_bot = self.local_da_head_bottom(x)
                        print(feat_bot.size())
                    else:
                    ...
                    # x.size() == [1024, 36, 62]
                    if self.context:
                        pred, feat_mid = self.da_head_mid(x)
                        print(feat_mid.size())
                        domain_pred.append(pred)
                    else:
                        domain_pred.append(self.da_head_mid(x))
           
                elif i == 3:
                    if self.context:
                        pred, feat_top = self.da_head_top(x)
                        print(feat_top.size())
                        domain_pred.append(pred)
                    else:
    ...
    def train(self, mode=True):
        ...
        freezed."""
        super(ResNet_DA_Tri_Att, self).train(mode)
        self._freeze_stages()
