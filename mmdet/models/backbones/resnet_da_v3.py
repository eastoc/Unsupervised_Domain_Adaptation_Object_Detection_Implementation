...
class ResNet_DA_v3(BaseModule):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(ResNet_DA_v3, self).__init__(init_cfg)
        ...
        # cycle-generator
        self.g_s2t = torch.load('Generator_s2t.pth')
        self.g_t2s = torch.load('Generator_t2s.pth')
        ...
        # cycle-discriminator
        self.D_s_img = torch.load('GAN_Discriminator_s.pth')
        self.D_t_img = torch.load('GAN_Discriminator_t.pt')
    ...
    def forward_train(self, x, gt_da):
        ...
        for i, img in enumerate(x):
            ...
            if gt_da[i] == 0:
                ...
                #print('rec tar',rec_tar.size)
        x = torch.cat(imgs, dim=0)
        
        dsn_t = torch.cat(dsn_t, dim=0)
        #print(x.size())
        dsn_logits_s = self.D_s_img(x) # discriminator logits
        ...
        return tuple(outs), domain_pred, local_feat, cycle_loss, dsn_logits_s, dsn_logits_t

    def train(self, mode=True):
        """Convert the model into training mode while keep normalization layer
        freezed."""
        super(ResNet_DA_v3, self).train(mode)
        self._freeze_stages()
