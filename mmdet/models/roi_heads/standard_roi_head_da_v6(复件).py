...
def bbox2roi_train(bbox_list):
    ...
    #print(rois.size())
    return rois, rois_list

@HEADS.register_module()
class StandardRoIHeadDA_v6(BaseRoIHeadDA, BBoxTestMixin, MaskTestMixin):
    ...
    def _bbox_forward_train(self, x, sampling_results, gt_bboxes, gt_labels, gt_da,
        ...
        if (gt_da[0] == 0) & (gt_da[1] == 0): # 都为source
            ...
            bbox_results = self._bbox_forward_da(x, rois) # 预测结果， dict(cls, bbox, features)
            bbox_targets = self.bbox_head.get_targets(sampling_results, gt_bboxes,
                                                      gt_labels, self.train_cfg)
           
        else:
            rois, rois_list = bbox2roi_train([res.bboxes for res in sampling_results])
            ...
            cls_score = torch.cat([bbox_results_src['cls_score'], bbox_results_tar['cls_score']], dim=0)
            bbox_pred = torch.cat([bbox_results_src['bbox_pred'], bbox_results_tar['bbox_pred']], dim=0)
            #print(bbox_targets)
