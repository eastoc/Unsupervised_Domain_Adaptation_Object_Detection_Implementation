...
class StandardRoIHeadDA_SWDA(BaseRoIHeadDA, BBoxTestMixin, MaskTestMixin):
    ...
    def _bbox_forward_train(self, x, sampling_results, gt_bboxes, gt_labels, gt_da,
        ...
        #loss_bbox = self.bbox_head.loss(bbox_results_src['cls_score'],
        #                                            bbox_results_src['bbox_pred'], roi_src,
        #                                            *bbox_targets)
        bbox_cls = []
        bbox_feats = [bbox_feat_src, bbox_feat_tar]
        #print(bbox_results_src['cls_score'])
        bbox_cls = [bbox_results_src['cls_score'], bbox_results_tar['cls_score']]
        bbox_results.update(loss_bbox=loss_bbox)
        return bbox_results, bbox_feats, bbox_cls
    ...
    def simple_test_tSNE(self,
        ...
        bbox_results = self.simple_test_bboxes_tSNE(
            x, img_metas, proposal_list, self.test_cfg, rescale=rescale)

        return bbox_results

    def simple_test_bboxes_tSNE(self,
                           x,
                           img_metas,
                           proposals,
                           rcnn_test_cfg,
