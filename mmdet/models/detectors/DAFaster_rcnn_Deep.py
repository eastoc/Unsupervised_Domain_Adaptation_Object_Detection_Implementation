...
from ..roi_heads.instance_da import InstanceAlignmentHead_DAF
import torch.nn.functional as F
from ..utils import cluster

@DETECTORS.register_module()
class DAFasterRCNN_Deep(TwoStageDetector):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(DAFasterRCNN_Deep, self).__init__(
            backbone=backbone,
            neck=neck,
            rpn_head=rpn_head,
            roi_head=roi_head,
        ...
        self.local_da_fore = InstanceAlignmentHead_DAF()
        self.local_da_fore._init_weights()

        self.local_da_back = InstanceAlignmentHead_DAF()
    ...
    def adaptive_loss_weights(self, global_loss=0, patch_mid_loss=0, patch_bot_loss=0, local_loss=0):
        ...
        #return global_lamdas, patch_lamda
    ...
    def forward_train(self,
        ...
        """
        if self.local_align:
            #local_da_loss = self.local_da_loss(bbox_feats, local_lamda)
        ...
        #global_lamdas, patch_mid_lamda, patch_bot_lamda, local_lamda = self.adaptive_loss_weights(global_loss, patch_mid_loss, patch_bottom_loss, local_da_loss)
        #global_lamdas, patch_lamda = self.adaptive_loss_weights(da_globle_loss, patch_da_loss)
        if self.global_align:
            #da_globle_loss = global_lamdas[0] * global_loss[0] + global_lamdas[1] * global_loss[1]
            da_globle_loss = global_lamda * sum(global_loss)
            losses.update(globle_da_loss=da_globle_loss)
        
        if self.patch_align:
            #patch_mid_loss = patch_mid_lamda * patch_mid_loss
            patch_bot_loss = patch_bot_lamda * patch_bottom_loss
            #losses.update(patch_mid_loss=patch_mid_loss)
            losses.update(patch_bottom_loss=patch_bot_loss)
        
        if self.local_align:
            local_da_loss = local_lamda * local_da_loss
            #print('ins',local_da_loss)
    ...
    def group(self, feats, cls_score, k=20):
        """
        ...
        if len(feats) > k:
            ...
            feats = self.complete(feats, cls_score, k)
            return feats
    ...
    def group_local_da_loss(self, bbox_feats ,lamda, bbox_cls):
        ...
        if len(fg_feat)!=0:
            ...
            #for i,feat in enumerate(fg_feat):
                #print(feat.unsqueeze(0).size())
            pred_da_fore = self.local_da_fore(fg_feat)

            #pred_da_fore = torch.cat(pred_da_fore, dim=0)
            ins_loss_fore = self.criterion(pred_da_fore, fore_gt)
        if len(bg_feat)!=0:
            #print(bg_feat.size())
            pred_da_back = []
            #for i,feat in enumerate(bg_feat):
            pred_da_back = self.local_da_back(bg_feat)
            #pred_da_back = torch.cat(pred_da_back, dim=0)
            ins_loss_back = self.criterion(pred_da_back, back_gt)
        # loss
