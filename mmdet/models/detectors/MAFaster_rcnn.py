...
class MAFasterRCNN(TwoStageDetector):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(MAFasterRCNN, self).__init__(
            backbone=backbone,
            neck=neck,
            rpn_head=rpn_head,
            roi_head=roi_head,
    ...
    def extract_feat_train(self, img, gt_domain):
        ...
        x, global_loss = self.backbone.forward_train(img, gt_domain)
        if self.with_neck:
            x = self.neck(x)
        return x, global_loss
    ...
    def forward_train(self,
        ...
        x, global_loss = self.extract_feat_train(img, gt_domain)
