...
class CyDAFasterRCNN(TwoStageDetector):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(CyDAFasterRCNN, self).__init__(
            backbone=backbone,
            neck=neck,
            rpn_head=rpn_head,
            roi_head=roi_head,
    ...
    def extract_feat_train(self, img, gt_da):
        ...
        x, da_label, patch_feat, cycle_loss, dsn_logits, dsn_logits_t = self.backbone.forward_train(img, gt_da)
        if self.with_neck:
            x = self.neck(x)
        return x, da_label, patch_feat, cycle_loss, dsn_logits, dsn_logits_t
    ...
    def forward_train(self,
        ...
        """
        global_lamda = 0.1 # 权重系数
        x, domain_pred, patch_feats, cycle_loss, dsn_logits_s, dsn_logits_t = self.extract_feat_train(img, gt_da)
        ...
        dsn_loss = self.criterion(dsn_logits_s, gt_domain)
        gan_loss = dsn_loss + self.criterion(dsn_logits_t, gt_domain)
        ...
        losses.update(gan_loss=gan_loss)
