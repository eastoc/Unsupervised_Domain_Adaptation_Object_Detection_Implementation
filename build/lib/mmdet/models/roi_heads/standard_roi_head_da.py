...
class StandardRoIHeadDA(BaseRoIHeadDA, BBoxTestMixin, MaskTestMixin):
    ...
    def _bbox_forward_da(self, x, rois):
        ...
        bbox_feats = self.bbox_roi_extractor(
            x[:self.bbox_roi_extractor.num_inputs], rois)
        bbox_feat_da = bbox_feats
        #print('bbox feat',bbox_feat_da.size())
        if self.with_shared_head:
            bbox_feats = self.shared_head(bbox_feats)
        cls_score, bbox_pred = self.bbox_head(bbox_feats)
        ...
        return bbox_results,bbox_feat_da
    ...
    def _bbox_forward_train(self, x, sampling_results, gt_bboxes, gt_labels, gt_da,
        ...
        if (gt_da[0] == 0) & (gt_da[1] == 0): # 都为source
            ...
            for i,roi in enumerate(rois): 
                if gt_da[i] == 0:
                    ...
                    bbox_results_src, bbox_feat_src = self._bbox_forward_da([x[0][i].unsqueeze(0)], roi)
                    ...
                    # instance-level判别器
                    instance_src = self.local_da(bbox_obj_feat_src)

                elif gt_da[i] == 1:
                    # 目标域预测结果， [dict(cls, bbox, features),features_da], (features 与features_da 的区别在于是否经过共享fc层)
                    bbox_results_tar,bbox_feat_tar = self._bbox_forward_da([x[0][i].unsqueeze(0)], roi)
                    ...
                    # instance-level判别器
                    instance_tar = self.local_da(bbox_obj_feat_tar)
            # 合并源域和目标域的预测结果
        ...
        loss_local_da = dict(loss_local_da=0.1*loss_local_da)  # 局部自适应损失 
        #print(loss_local_da)
        #print(loss_bbox)
