...
class StandardRoIHeadDA_v2(BaseRoIHeadDA, BBoxTestMixin, MaskTestMixin):
    ...
    def _bbox_forward_train(self, x, sampling_results, gt_bboxes, gt_labels, gt_da,
        ...
        bbox_results = self._bbox_forward_da(x, rois)
        bbox_targets = self.bbox_head.get_targets([sampling_results[src_idx]], [gt_bboxes[0][src_idx]],
                    [gt_labels[0][src_idx]], self.train_cfg)

        bbox_feat_src = bbox_results['bbox_feats'][src_idx]
        bbox_feat_tar = bbox_results['bbox_feats'][tar_idx]
        bbox_feats = [bbox_feat_src, bbox_feat_tar]
        #print(bbox_results['cls_score'].size())
        loss_bbox = self.bbox_head.loss(bbox_results['cls_score'][0:511], bbox_results['bbox_pred'][0:511], rois[src_idx], *bbox_targets)
