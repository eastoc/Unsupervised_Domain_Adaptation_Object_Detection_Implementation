...
class StandardRoIHeadDA_v3(BaseRoIHeadDA, BBoxTestMixin, MaskTestMixin):
    ...
    def _bbox_forward_train(self, x, sampling_results, gt_bboxes, gt_labels, gt_da,
        ...
        #creteria_fl = FocalLoss(gamma=2, alpha=0.25)
        ...
        if (gt_da[0] == 0) & (gt_da[1] == 0): # 都为source
            ...
            for i,roi in enumerate(rois): 
                if gt_da[i] == 0:
                    ...
                    #instance_tar = None
                    #loss_local_da = loss_local_da + loss
            label_src = torch.zeros(len(bbox_feat_src), requires_grad=True, device='cuda').long()
            label_tar = torch.ones(len(bbox_feat_tar), requires_grad=True, device='cuda').long()
            label_da = torch.cat([label_src, label_tar], dim=0)

            bbox_feats = torch.cat([bbox_feat_src, bbox_feat_tar], dim=0)
            pred_da = self.local_da(bbox_feats)

            loss_local_da = creteria(pred_da, label_da)

            #loss_bbox = self.bbox_head.loss(bbox_results_src['cls_score'], bbox_results_src['bbox_pred'], roi_src, *bbox_targets)
        '''
            # 合并源域和目标域的预测结果
        ...
        for i,da_pred in enumerate(instance_da):
            if gt_da[i] == 0:
                ...
                loss_local_da = loss
        '''
        loss_local_da = loss_local_da * 0.2
