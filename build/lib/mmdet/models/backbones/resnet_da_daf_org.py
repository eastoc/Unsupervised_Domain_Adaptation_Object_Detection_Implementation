...
class SRM(BaseModule):
    def __init__(self, in_channel):
        super(SRM, self).__init__()
        
        self.output_channel = int(in_channel/4)*3*3
        self.grl = GradientScalarLayer(-1.0)
        self.conv1 = nn.Conv2d(in_channels=in_channel, out_channels=int(in_channel/4), kernel_size=1, padding=1, stride=1)
        self.bn1 = nn.BatchNorm2d(int(in_channel/4))
        self.conv2 = nn.Conv2d(in_channels=int(in_channel/4), out_channels=self.output_channel, kernel_size=3, padding=3)
...
class ImgAlignmentHead(BaseModule):
    def __init__(self, in_channel):
        super(ImgAlignmentHead, self).__init__()
        
        self.grl = GradientScalarLayer(-1.0)
        self.conv1 = nn.Conv2d(in_channels=in_channel, out_channels=512, kernel_size=1, stride=1, padding=0)
        self.conv2 = nn.Conv2d(in_channels=512, out_channels=1, kernel_size=1, stride=1, padding=0)
     
    def forward(self, x):
        x = self.grl(x)
        x = F.relu(self.conv1(x))
        img_feat = F.relu(self.conv2(x))
    ...
    def _init_weights(self):
        ...
        normal_init(self.conv2, 0, 0.001)
...
class ResNet_DAF(BaseModule):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(ResNet_DAF, self).__init__(init_cfg)
        ...
        self.da_head_top = ImgAlignmentHead(in_channel=2048)
    ...
    def forward_train(self, x, gt_domain):
        ...
        for i, layer_name in enumerate(self.res_layers):
            ...
            if i in self.da_outs_idx:
                if i in self.out_indices:
                    outs.append(x)
                if i == 3:
                    patch_feat = self.da_head_top(x)
                    
        patch_da_bottom_loss = []
        for i in range(len(patch_feat)):
            if gt_domain[i] == 0:
        ...
        return tuple(outs), patch_da_bottom_loss, patch_feat

    def train(self, mode=True):
        """Convert the model into training mode while keep normalization layer
        freezed."""
        super(ResNet_DAF, self).train(mode)
        self._freeze_stages()
