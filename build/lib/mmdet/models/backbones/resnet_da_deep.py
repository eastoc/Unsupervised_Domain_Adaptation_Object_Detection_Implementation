...
class shared_da_conv_head(nn.Module):
    ...
    def forward(self, inputs):
        res = self.grl(inputs)
        x = self.drop(F.relu(self.bn1(self.conv1(res))))
...
class ResNet_DA_Deep(BaseModule):
    """ResNet DA backbone.
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(ResNet_DA_Deep, self).__init__(init_cfg)
        ...
        self.local_da_head_mid = NonLocalAlignmentHead(in_channels=1024, context=False, grl=True)
        self.local_da_head_bottom = NonLocalAlignmentHead(in_channels=512, context=False)
    ...
    def forward_train(self, x, gt_domain):
        ...
        if self.patch_bottom_align:
            ...
            patch_da_loss = patch_da_bottom_loss + patch_da_loss
        ...
        if self.patch_mid_align:
            ...
            patch_da_loss = patch_da_mid_loss + patch_da_loss

        return tuple(outs), da_globle_loss, patch_da_loss

    def train(self, mode=True):
        """Convert the model into training mode while keep normalization layer
        freezed."""
        super(ResNet_DA_Deep, self).train(mode)
        self._freeze_stages()
